...
class partner(models.Model):
	...
	category = models.CharField(max_length=32, default='None', null=True, blank=True)
	...
	def phoneNumber(self):
		return '(' + self.phone[0:3] + ') ' + self.phone[3:6] + '-' + self.phone[6:]

	def contactPhoneNumber(self):
		if (self.contact_phone):
			if len(self.contact_phone) is 10:
				return '(' + self.contact_phone[0:3] + ') ' + self.contact_phone[3:6] + '-' + self.contact_phone[6:10]		
			else:
