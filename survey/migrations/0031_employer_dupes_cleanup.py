...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        dupes_dict = dict(EMPLOYER_DUPES)
        for c in orm.Commutersurvey.objects.filter(employer__in=[d[0] for d in EMPLOYER_DUPES]):
            new_employer_id = dupes_dict[c.employer.id]
            c.employer = orm.Employer.objects.get(pk=new_employer_id)
            c.save()

        # delete old employers
        orm.Employer.objects.filter(pk__in=[d[0] for d in EMPLOYER_DUPES]).delete()
