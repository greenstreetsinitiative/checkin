...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for c in orm.Commutersurvey.objects.filter(month__isnull=False):
            try:
                m = orm.Month.objects.get(wr_day=datetime.datetime.strptime(c.month, '%B %Y'))
                c.wr_day_month = m
                c.save()
            except orm.Month.DoesNotExist:
                m = orm.Month()
                month = '01 %s' % c.month
                m.wr_day = datetime.datetime.strptime(month, '%d %B %Y')
                m.active = False
                m.save()
                c.wr_day_month = m
                c.save()
    ...
    def backwards(self, orm):
        ...
        for c in orm.Commutersurvey.objects.filter(wr_day__isnull=False):
            c.month = c.wr_day.month
            c.save()
