...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Note: you can't call static model methods in south migrations...
        LEG_DIRECTIONS = (
            ('tw', _('to work')),
            ('fw', _('from work')),
        )
        ...
        # set default value for legacy Leg.duration
        orm.Leg.objects.filter(duration__isnull=True).update(duration=1)

        for c in orm.Commutersurvey.objects.all():
            ...
            for direction in [d[0] for d in LEG_DIRECTIONS]:
                ...
                for day in [d[0] for d in LEG_DAYS]:
                    duration = legs.exclude(mode__in=['c', 'da']).filter(direction=direction, day=day).aggregate(Sum('duration'))
            ...
            c.from_work_switch = switch['fw']
            c.to_work_switch = switch['tw']
            c.save()
