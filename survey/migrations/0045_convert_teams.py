...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # these employers are actually subteams
        subteams = orm.Employer.objects.filter(is_parent=False, active=True,sector__gt=9)

        for subteam in subteams:
            s = subteam.sector.id
            sector = orm.EmplSector.objects.get(id=s)
            e = orm.Employer.objects.filter(name=sector.parent, is_parent=True).first()
            newteam = orm.Team.objects.create(name=subteam.name, company=e)
