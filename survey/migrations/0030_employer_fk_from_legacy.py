...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for c in orm.Commutersurvey.objects.filter(employer_legacy__isnull=False):
            try:
                e = orm.Employer.objects.get(name=c.employer_legacy)
                c.employer = e
                c.save()
            except orm.Employer.DoesNotExist:
                pass

    def backwards(self, orm):
        "Write your backwards methods here."
        for c in orm.Commutersurvey.objects.filter(employer__isnull=False):
            c.employer_legacy = c.employer.name
            c.save()
